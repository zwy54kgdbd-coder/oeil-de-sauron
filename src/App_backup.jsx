...
function App() {
  ...
  useEffect(() => {
    const savedIdentites = localStorage.getItem("identites");
    const savedUsers = localStorage.getItem("users");
    const savedVehicules = localStorage.getItem("vehicules");
    const savedHistorique = localStorage.getItem("historique");

    if (savedIdentites) setIdentites(safeParseArray(savedIdentites));
    if (savedUsers) {
      const loadedUsers = safeParseArray(savedUsers);
      setUsers(loadedUsers.length > 0 ? loadedUsers : initialUsers);
    }
    if (savedVehicules) setVehicules(safeParseArray(savedVehicules));
    if (savedHistorique) setHistorique(safeParseArray(savedHistorique));
  }, []);
  ...
  const enregistrerIdentite = () => {
    ...
    const fiche = {
      ...
      photos,
      photoPrincipaleIndex,
    };

    const updated = editingId
      ? identites.map((item) => (item.id === editingId ? fiche : item))
      : [...identites, fiche];

    saveIdentites(updated);
  ...
  const supprimerIdentite = (id) => {
    if (String(id).startsWith("fake")) {
      alert("Fiche test non supprimable.");
      return;
    }

    const updated = identites.filter((item) => item.id !== id);
    saveIdentites(updated);

    const vehiculesMisAJour = vehicules.map((item) =>
      String(item.individuId) === String(id)
        ? { ...item, individuId: "" }
        : item
    );
    saveVehicules(vehiculesMisAJour);
  ...
  const enregistrerVehicule = () => {
    ...
    if (vehiculeIndividuId === CREATE_NEW_IDENTITY) {
      ...
      const nouvelleIdentite = {
        ...
        photo: "",
        photos: [],
        photoPrincipaleIndex: 0,
      };

      const identitesUpdated = [...identites, nouvelleIdentite];
      saveIdentites(identitesUpdated);
      finalIndividuId = nouvelleIdentite.id;

      ajouterHistorique(
        `Création identité depuis véhicule : ${nouvelleIdentiteNom} ${nouvelleIdentitePrenom}`
      );
    }

    const ficheVehicule = {
      id: editingVehiculeId || Date.now(),
    ...
    const updated = editingVehiculeId
      ? vehicules.map((item) =>
          String(item.id) === String(editingVehiculeId) ? ficheVehicule : item
        )
      : [...vehicules, ficheVehicule];

    saveVehicules(updated);
  ...
  const supprimerVehicule = (id) => {
    const updated = vehicules.filter((item) => item.id !== id);
    saveVehicules(updated);
